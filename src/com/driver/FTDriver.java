...
public class FTDriver {
    ...
    public static final int WRITEBUF_SIZE = 13;
    ...
    public boolean begin(int baudrate) {
        for (UsbDevice device : mManager.getDeviceList().values()) {
        	...
        	//Toast.makeText(FTDriver.this, "USBDevice " + mManager.getDeviceList().values(), Toast.LENGTH_SHORT).show();
            Log.i(TAG, "Devices : " + device.toString());

            if (!mManager.hasPermission(device)) {
            	getPermission(device);   //-- sudhir
            }
            if (!mManager.hasPermission(device)) {
                return false;
            }
       ...
       // Toast.makeText(Context, mDeviceConnection.getSerial(), Toast.LENGTH_SHORT).show();
    ...
    private boolean setUSBInterface(UsbDevice device, UsbInterface intf,
        ...
        if (device != null && intf != null) {
            ...
            if (connection != null) {
                ...
                //Toast.makeText(this, "Open Succeeded", Toast.LENGTH_SHORT).show();
